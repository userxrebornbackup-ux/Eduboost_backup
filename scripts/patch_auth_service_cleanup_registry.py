...
def main() -> int:
    status = write_status()
    proof = "runtime-passing" if status.status != "auth-service-cleanup-not-proven" else "not-proven"
    blocker = "; ".join(status.blockers) or "none"
    block = f"""  - id: AUTH-SERVICE-CLEANUP-001
    title: Auth service monkey-patch cleanup and route ownership guardrails
    severity: P1
    gate: 3
    owner: backend
    implementation_batch: code_2511_2550
    proof_status: {proof}
    proof_command: make backend-implementation-2511-2550-full-check
    evidence_file: docs/release/auth_service_cleanup_status.md
...
"""
    ...
    REGISTRY.write_text(replace_or_append(text, "AUTH-SERVICE-CLEANUP-001", block), encoding="utf-8")
    print("Updated AUTH-SERVICE-CLEANUP-001 registry entry")
    return 0
