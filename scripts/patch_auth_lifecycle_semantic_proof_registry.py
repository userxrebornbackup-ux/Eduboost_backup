...
def main() -> int:
    status = write_status()
    proof = "runtime-passing" if status.status == "auth-lifecycle-controlled-semantic-proof-passing" else "not-proven"
    blocker = "; ".join(status.blockers) or "none"
    block = f"""  - id: AUTH-LIFECYCLE-SEMANTIC-PROOF-001
    title: Auth lifecycle controlled semantic proof
    severity: P1
    gate: 3
    owner: backend
    implementation_batch: code_2631_2670
    proof_status: {proof}
    proof_command: make backend-implementation-2631-2670-full-check
    evidence_file: docs/release/auth_lifecycle_semantic_proof_status.md
...
"""
    ...
    REGISTRY.write_text(replace_or_append(text, "AUTH-LIFECYCLE-SEMANTIC-PROOF-001", block), encoding="utf-8")
    print("Updated AUTH-LIFECYCLE-SEMANTIC-PROOF-001 registry entry")
    return 0
