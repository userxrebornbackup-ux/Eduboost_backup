...
def popia_slice_block() -> str:
    report = write_report()
    proof_status = "runtime-passing" if report.local_status == "route-popia-delegation-passing" else "not-proven"
    return f"""  - id: ROUTE-TX-POPIA-001
    title: POPIA route transaction slice
    severity: P1
    gate: 4
    owner: backend
    implementation_batch: code_2111_2150
    proof_status: {proof_status}
    proof_command: make backend-implementation-2111-2150-full-check
    evidence_file: docs/release/popia_route_transaction_slice_report.md
...
def main() -> int:
    ...
    text = replace_or_append(text, "ROUTE-TX-POPIA-001", popia_slice_block())
    REGISTRY.write_text(text, encoding="utf-8")
    print("Updated ROUTE-TX-POPIA-001 registry entry")
    return 0
