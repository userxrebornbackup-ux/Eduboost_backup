...
- [x] `[high]` Add graceful degraded-mode behavior for optional dependencies such as LLM providers, analytics, email, or billing. Evidence: `app/core/degraded_mode.py`, `/api/v2/system/capabilities`, telemetry no-op behavior, and billing capability guard.
...
- [ ] `[high]` Add product overview, parent guide, learner guide, teacher guide, FAQ, pricing/billing FAQ, and AI transparency FAQ. Partial evidence: `docs/product/launch_scope.md`, `docs/product/pricing_operations.md`, `docs/product/roadmap.md`, and `docs/product/differentiation_strategy.md`; user-facing guides/FAQ remain pending.
...
- [x] `[critical]` Define analytics events with privacy review. Evidence: `docs/analytics/learning_event_schema.md`, `app/services/telemetry.py`, and analytics allowlist tests.
- [x] `[critical]` Do not send raw learner PII to analytics tools. Evidence: `app/services/telemetry.py` rejects direct email identifiers and allowlists event properties.
- [ ] `[high]` Track core funnel: visit, signup started, signup completed, learner added, consent granted, diagnostic started, diagnostic completed, first lesson opened, first practice completed, parent report viewed. Partial evidence: `app/core/analytics.py` and `docs/analytics/learning_event_schema.md`; full funnel instrumentation remains pending.
- [ ] `[high]` Track learning outcomes: baseline diagnostic, remediation activity, reassessment, mastery change. Partial evidence: `docs/analytics/learning_event_schema.md`; full product instrumentation remains pending.
- [x] `[medium]` Add analytics opt-out where appropriate and event schema validation. Evidence: telemetry is no-op when analytics provider is disabled and `validate_event_payload` enforces schema/PII constraints.
...
- [x] `[research]` Define ethical experimentation guidelines for minors and education. Evidence: `docs/experimentation/ethics_for_minors.md`.
...
- [x] `[medium]` Fall back from live generation to curated/static content. Evidence: `app/core/degraded_mode.py` declares LLM fallback and existing LLM gateway offline fallback remains supported.
...
- [x] `[high]` Define learning event schema: pseudonymous learner ID, topic, activity type, timestamp, correctness, attempt number, time spent, hint usage, confidence. Evidence: `docs/analytics/learning_event_schema.md`.
- [ ] `[high]` Separate analytics warehouse from transactional DB if scale requires.
- [x] `[high]` Add anonymization or pseudonymization strategy. Evidence: `docs/analytics/learning_event_schema.md` and `app/services/telemetry.py` require pseudonymous identifiers and property allowlisting.
...
- [x] `[high]` Ensure gamification supports learning, not compulsive engagement. Evidence: `docs/experimentation/ethics_for_minors.md` and `docs/product/differentiation_strategy.md` prohibit dark patterns/compulsive mechanics.
- [x] `[high]` Avoid dark patterns. Evidence: `docs/experimentation/ethics_for_minors.md` and `docs/product/differentiation_strategy.md`.
- [ ] `[high]` Make mastery-based rewards more important than time-spent rewards. Partial evidence: product/experimentation docs define the rule; implementation remains pending in gamification scoring.
...
- [x] `[critical]` Define supported launch scope: grades, subjects, terms, topics. Evidence: `docs/product/launch_scope.md` and `docs/curriculum/caps_topic_map.md`.
- [x] `[critical]` Do not claim complete CAPS coverage until verified. Evidence: `docs/product/launch_scope.md` and beta UI label state limited scope.
- [ ] `[high]` Create coverage matrix.
- [ ] `[high]` Add minimum lesson count and diagnostic item count per topic.
- [x] `[medium]` Add curriculum gap roadmap. Evidence: `docs/product/roadmap.md` and `app/services/curriculum/coverage.py`.
...
- [ ] `[critical]` Draft and review Terms of Service, Privacy Policy, Child-friendly Privacy Notice, Parent Consent Notice, Acceptable Use Policy, Refund Policy, and school Data Processing Agreement. Partial evidence: `docs/legal/legal_documents_index.md`; final legal drafting/review remains external.
- [ ] `[critical]` Ensure legal docs match actual product behavior. Partial evidence: `docs/legal/legal_documents_index.md` assigns product-behavior verification before publication; legal review remains external.
- [x] `[high]` Add legal document versioning and acceptance tracking. Evidence: `docs/legal/policy_versioning.md` defines semantic versioning, acceptance metadata, and notification workflow.
- [x] `[high]` Add policy update notification workflow. Evidence: `docs/legal/policy_versioning.md`.
- [x] `[high]` Define production support model, pricing operations, refund process, and escalation process. Evidence: `docs/operations/support_model.md` and `docs/product/pricing_operations.md`.
...
- [x] `[critical]` Use synthetic or explicitly consented test users only. Evidence: `docs/product/launch_scope.md` and release/alpha gate docs.
- [ ] `[critical]` Validate onboarding, consent, diagnostics, lessons, parent reports, and support workflow.
- [x] `[high]` Collect structured feedback from parents, learners, and educators. Evidence: beta feedback/report issue controls added in frontend and support model defines content correction triage.
- [x] `[high]` Measure technical stability and content correctness. Evidence: PR-008 observability docs plus `docs/operations/support_model.md` content correction workflow.
...
- [x] `[high]` Add beta label in UI. Evidence: `app/frontend/src/components/eduboost/BetaAndFeedback.tsx`, homepage, shell, and lesson views.
- [x] `[high]` Add feedback/report issue button. Evidence: `app/frontend/src/components/eduboost/BetaAndFeedback.tsx`, learner shell, and lesson view.
- [ ] `[high]` Add content correction workflow.
- [x] `[medium]` Publish transparent roadmap. Evidence: `docs/product/roadmap.md`.
...
- [x] `[high]` Announce limited scope accurately. Evidence: `docs/product/launch_scope.md` and private-beta limited-scope UI label.
- [x] `[high]` Avoid overclaiming AI capability, CAPS coverage, or guaranteed learning outcomes. Evidence: `docs/product/launch_scope.md` and `docs/product/differentiation_strategy.md`.
...
- [ ] `[medium]` Use the graph to power next-best lesson, parent explanations, teacher intervention groups, and curriculum coverage maps. Partial evidence: `docs/product/differentiation_strategy.md`; graph implementation remains future work.
...
- [x] `[medium]` Treat low-data mode as a first-class product feature. Evidence: `docs/product/differentiation_strategy.md` and `docs/product/roadmap.md`.
...
- [x] `[medium]` Avoid negative labels such as “weak learner” or “poor performer.” Evidence: `docs/analytics/learning_event_schema.md`.
- [x] `[medium]` Use constructive labels: ready to strengthen, needs another example, almost mastered, ready for challenge. Evidence: `docs/analytics/learning_event_schema.md`.
- [ ] `[medium]` Add explainable recommendations: “We recommend fractions because the diagnostic showed difficulty with equivalent forms.” Partial evidence: analytics/product docs define explainability rule; recommendation engine remains pending.
...
- [x] Feedback/report content issue. Evidence: `BetaAndFeedback` component in learner shell and lesson view.
...
- [ ] Legal docs. Partial evidence: `docs/legal/legal_documents_index.md`; final legal docs need review.
- [ ] Privacy docs.
- [x] Incident response. Evidence: `docs/incident_response.md`, `SECURITY.md`, and `docs/operations/support_model.md`.
- [x] Backups. Evidence: PR-008 backup/restore scripts and runbook; live restore drill remains external.
- [x] Monitoring. Evidence: PR-008 Prometheus/Grafana/alerts/runbook assets.
- [x] Support workflow. Evidence: `docs/operations/support_model.md`.
- [ ] E2E tests.
- [ ] First release tag.
- [ ] Limited beta cohort.
...
- [x] Low-data mode. Evidence: `docs/product/differentiation_strategy.md` and `docs/product/roadmap.md`.
- [ ] Parent co-pilot.
- [ ] Teacher dashboard.
- [ ] Sponsored learner model.
- [ ] CAPS learning graph. Partial evidence: `docs/product/differentiation_strategy.md`; implementation remains future work.
