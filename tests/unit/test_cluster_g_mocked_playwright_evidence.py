...
@pytest.mark.unit
def test_cluster_g_mocked_playwright_evidence_registered() -> None:
    subprocess.run(
...
@pytest.mark.unit
def test_cluster_g_workflow_runs_mocked_playwright_checks() -> None:
    text = WORKFLOW.read_text(encoding="utf-8")

    assert "make frontend-playwright-mock-helper-check" in text
    assert "make frontend-playwright-mocked-specs-check" in text
    assert "tests/unit/test_frontend_playwright_mock_helpers.py" in text
    assert "tests/unit/test_frontend_playwright_mocked_specs.py" in text
